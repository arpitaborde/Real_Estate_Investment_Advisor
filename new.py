...
@st.cache_data
def load_data():
    return pd.read_csv(r"C:\Users\AB\Desktop\housing_cleaned.csv")
...
if page == "Overview":
    ...
    with col2:
        st.metric("Avg Price (Lakhs)", round(df["Price_in_Lakhs"].mean(), 2))

    with col3:
        st.metric("Avg Size (SqFt)", round(df["Size_in_SqFt"].mean(), 0))
